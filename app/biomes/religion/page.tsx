...
import { ChevronLeft, BookOpen, Map, Puzzle, FileText, Microscope, Scroll, Crown } from 'lucide-react'
import { religionLearningPath } from '@/lib/religion-data'
import { getUserData, getTotalXP } from '@/lib/xp'

type StationId = 'religion-training-camp' | 'theology-lab' | 'religion-quiz' | 'religion-boss'

export default function ReligionBiomePage() {
  ...
  const getStationIcon = (id: StationId) => {
    switch (id) {
      case 'religion-training-camp': return <Map className="w-6 h-6" />
      case 'theology-lab': return <Microscope className="w-6 h-6" />
      case 'religion-quiz': return <Scroll className="w-6 h-6" />
      case 'religion-boss': return <Crown className="w-6 h-6" />
    }
  }

  const isStationCompleted = (id: string) => completedLevels.includes(id)
  const isStationLocked = (station: typeof religionLearningPath.stations[0]) => {
    ...
    <div className="min-h-screen bg-gradient-to-b from-amber-900 via-orange-900 to-amber-950">
      <div className="container mx-auto px-4 py-8">
        ...
        >
          ...
          <div className="text-center">
            ...
            <p className="text-amber-200 max-w-xl mx-auto">
              Das Alte Testament verstehen: Von der Schöpfung über den Bund bis zu den Propheten
            </p>
          </div>
        </motion.div>
        ...
        >
          ...
          <p className="text-amber-200 text-sm mt-2 text-center">
        ...
        <div className="max-w-2xl mx-auto space-y-4">
          {religionLearningPath.stations.map((station, index) => {
                    ...
                    <div 
                      className={`group relative overflow-hidden rounded-2xl p-6 cursor-pointer
                        ...
                        ${completed 
                          ...
                          : 'bg-gradient-to-r from-amber-600/30 to-orange-600/30 border-2 border-amber-400/50 hover:border-amber-300'
                        }`}
                    >
                      <div className="flex items-center gap-4">
                        <div className={`p-3 rounded-xl ${completed ? 'bg-green-500/30' : 'bg-amber-500/30'}`}>
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-amber-200/80 text-sm mt-1">{station.description}</p>
                          
                          <div className="flex items-center gap-4 mt-2">
                            <span className="text-yellow-400 text-sm font-medium">+{station.totalXP} XP</span>
                            {!completed && <span className="text-amber-300 text-sm">Klicke zum Starten →</span>}
                          </div>
                        </div>
                      </div>
                    </div>
